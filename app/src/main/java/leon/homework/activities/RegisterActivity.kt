...
class RegisterActivity : BaseActivity(), View.OnClickListener, Handler.Callback{
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        initSDK()
        AlertDialog.Builder(this@RegisterActivity).setMessage("仅提供学生进行注册\n教师账号密码请联系学校管理获取")
    ...
    override fun onClick(v: View) {
        when (v.id) {
            R.id.btn_verification -> {
                loading(Const.SHOW)
                getVerificationCode("86", et_phonenumber.text.toString())
            }
            R.id.btn_confirm -> {
                if(iscreateable){
                    loading(Const.SHOW)
    ...
    override fun handleMessage(msg: Message): Boolean {
        ...
        if (result == SMSSDK.RESULT_COMPLETE) {
            ...
            try {
                data = msg.obj as Throwable
                Json = JSONObject(data.message)
